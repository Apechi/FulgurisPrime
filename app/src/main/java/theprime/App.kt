...
import theprime.activity.IncognitoActivity
...
import theprime.settings.preferences.DeveloperPreferences
import theprime.settings.preferences.LandscapePreferences
import theprime.settings.preferences.PortraitPreferences
import theprime.settings.preferences.UserPreferences
import theprime.utils.installMultiDex
...
import theprime.settings.preferences.ConfigurationPreferences
...
class App : Application(), SharedPreferences.OnSharedPreferenceChangeListener,
    ...
    override fun onActivityDestroyed(activity: Activity) {
        ...
        theprime.utils.MemoryLeakUtils.clearNextServedView(activity, this@App)
    }
    ...
    override fun onCreate() {
        ...
        Thread.setDefaultUncaughtExceptionHandler { thread, ex ->
            if (userPreferences.crashLogs) {
                theprime.utils.FileUtils.writeCrashToStorage(ex)
            }
	...
	// TODO: Remove that once we are done with ReactiveX
        RxJavaPlugins.setErrorHandler { throwable: Throwable? ->
            if (userPreferences.crashLogs && throwable != null) {
                theprime.utils.FileUtils.writeCrashToStorage(throwable)
                throw throwable
            }
        }

        // Apply locale
        val requestLocale = theprime.locale.LocaleUtils.requestedLocale(userPreferences.locale)
        theprime.locale.LocaleUtils.updateLocale(this, requestLocale)
    ...
    companion object {
         ...
         */
        fun setLocale() {
            val requestLocale = theprime.locale.LocaleUtils.requestedLocale(app.userPreferences.locale)
            theprime.locale.LocaleUtils.updateLocale(app, requestLocale)
        }
