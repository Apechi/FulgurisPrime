...
import theprime.constant.INTENT_ORIGIN
import theprime.extensions.snackbar
import theprime.search.SearchEngineProvider
import theprime.settings.NewTabPosition
...
import theprime.Component
import theprime.utils.QUERY_PLACE_HOLDER
import theprime.utils.isBookmarkUrl
import theprime.utils.isDownloadsUrl
import theprime.utils.isHistoryUrl
import theprime.utils.isIncognitoPageUrl
import theprime.utils.isSpecialUrl
import theprime.utils.isStartPageUrl
import theprime.utils.smartUrlFilter
import theprime.view.BookmarkPageInitializer
import theprime.view.DownloadPageInitializer
import theprime.view.FreezableBundleInitializer
import theprime.view.HistoryPageInitializer
import theprime.view.HomePageInitializer
import theprime.view.IncognitoPageInitializer
import theprime.view.NoOpInitializer
import theprime.view.TabInitializer
import theprime.view.UrlInitializer
import theprime.view.WebPageTab
...
class TabsManager @Inject constructor(
    ...
    */
    ...
    {
        val bundle = theprime.utils.FileUtils.readBundleFromStorage(application, aFilename)
    ...
    fun renameSession(aOldName: String, aNewName: String) {
        ...
        theprime.utils.FileUtils.renameBundleInStorage(application, fileNameFromSessionName(oldName), fileNameFromSessionName(aNewName))
...
//        currentTab?.pauseTimers()
    ...
    private fun saveCurrentSession(aName: String) {
        ...
        iScopeThreadPool.launch {
            ...
            theprime.utils.FileUtils.writeBundleToStorage(application, outState, temp)
            // Defensively delete session backup, that should never be needed really
            theprime.utils.FileUtils.deleteBundleInStorage(application, backup)
            // Rename our session file as backup
            theprime.utils.FileUtils.renameBundleInStorage(application, session, backup)
            // Rename our temporary session to actual session
            theprime.utils.FileUtils.renameBundleInStorage(application, temp, session)
...
//            }
     ...
     * browser next starts.
     */
    fun clearSavedState() = theprime.utils.FileUtils.deleteBundleInStorage(application, FILENAME_SESSION_DEFAULT)
    ...
    fun deleteSession(aSessionName: String) {
        ...
        // Delete session file
        theprime.utils.FileUtils.deleteBundleInStorage(application, fileNameFromSessionName(iSessions[index].name))
