...
import theprime.R
import theprime.browser.ProxyChoice
import theprime.browser.SuggestionNumChoice
import theprime.constant.TEXT_ENCODINGS
import theprime.constant.Uris
import theprime.di.*
import theprime.dialog.BrowserDialog
import theprime.extensions.resizeAndShow
import theprime.extensions.withSingleChoiceItems
import theprime.isSupported
import theprime.search.SearchEngineProvider
import theprime.search.Suggestions
import theprime.search.engine.BaseSearchEngine
import theprime.search.engine.CustomSearch
import theprime.settings.preferences.*
...
import theprime.settings.preferences.USER_AGENTS_ORDERED
import theprime.settings.preferences.USER_AGENT_CUSTOM
...
class GeneralSettingsFragment : AbstractSettingsFragment() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        findPreference<ListPreference>(getString(R.string.pref_key_locale))?.apply {
            onPreferenceChangeListener = Preference.OnPreferenceChangeListener { _, aNewLocale: Any ->
                ...
                val newLocale = theprime.locale.LocaleUtils.requestedLocale(newLocaleId)
                // Update app configuration with selected locale
                theprime.locale.LocaleUtils.updateLocale(activity, newLocale)
                // Reload our activity
    ...
    private fun updateProxyChoice(choice: ProxyChoice, activity: Activity, summaryUpdater: SummaryUpdater) {
        val sanitizedChoice = theprime.utils.ProxyUtils.sanitizeProxyChoice(choice, activity)
    ...
    private fun showDownloadLocationDialog(summaryUpdater: SummaryUpdater) : Boolean {
        activity?.let {
            ...
            setSingleChoiceItems(R.array.download_folder, n) { _, which ->
                when (which) {
                    0 -> {
                        userPreferences.downloadDirectory = theprime.utils.FileUtils.DEFAULT_DOWNLOAD_PATH
                        summaryUpdater.updateSummary(theprime.utils.FileUtils.DEFAULT_DOWNLOAD_PATH)
                    }
                    1 -> {
        ...
        activity?.let { activity ->
            ...
            BrowserDialog.showCustomDialog(activity) {
                ...
                setPositiveButton(R.string.action_ok) { _, _ ->
                    ...
                    text = theprime.utils.FileUtils.addNecessarySlashes(text)
    ...
    private class DownloadLocationTextWatcher(
        ...
        override fun afterTextChanged(s: Editable) {
            if (!theprime.utils.FileUtils.isWriteAccessAvailable(s.toString())) {
